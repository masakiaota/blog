...
class BaseDecisionTree(BaseEstimator, MultiOutputMixin, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None, check_input=True,
        ...
        # splitterをdenseとsparseで使い分けてる(計算の効率化のため？)
        SPLITTERS = SPARSE_SPLITTERS if issparse(X) else DENSE_SPLITTERS

        splitter = self.splitter  # そのままsplitterがSplitterインスタンスなら問題なし
        if not isinstance(self.splitter, Splitter):
            # インスタンスでないならば
            # _splitter.BestSplitter or _splitter.BestSparseSplitterのインスタンスが生成される。
            # SPLITTERS["best"]となる
        ...
        if max_leaf_nodes < 0:
            # max_leaf_nodesを指定しないので、われわれが使うbuilderはこっち
    ...
    # ここで特徴量の重要度を得られるが、考慮されてないやつと考慮されたが選ばれていないやつは等しく0となるので、これが問題。
