...
def data_pattern8(random_state=2019):
    '''
    get artificial dataset


    同じ分布(正規分布とか)で分散も同じだが、ある程度集約する場合(binを50個、20個、10個、3個みたいに変えてみる)
    3シグマ区間を端として集計する
    
    最後の特徴量（一番荒い）だけラベルに関連付ける
    ...
    # bins=10の場合
    partations, representative=_get_partations_and_representatives(10,sigma)
    norm_tmp=norm.rvs(loc=0.5, scale=sigma,size=(100,1))
    norm_tmp=np.vstack([norm_tmp, norm.rvs(loc=-0.5, scale=sigma,size=(100,1))]).reshape(-1)
#     norm_tmp=norm.rvs(loc=0, scale=sigma,size=(200,))
    for left,right,rep in zip(partations[:-1],partations[1:],representative):
        norm_tmp[(left<=norm_tmp)&(norm_tmp<right)]=rep
